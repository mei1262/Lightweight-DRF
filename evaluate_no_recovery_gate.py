...
model_path = "best_no_recovery_gate.pth"
...
def main():
    ...
    model = NoRecoveryGate(num_classes=3, hard_gate=True, text_gate_threshold=0.35).to(device)
    model.load_state_dict(torch.load(model_path, map_location=device))
    model.eval()

    # clean
    loader = DataLoader(MVSADataset(test_json, image_dir, tokenizer, is_train=False), batch_size=16, shuffle=False)
    ...
    for modality in ["image", "text"]:
        for dtype in [DisruptionType.C, DisruptionType.D, DisruptionType.C_D]:
            ds = RobustMVSADataset(
                ...
                strategy=DisruptionStrategy.FIXED, disruption_type=dtype,
                disruption_ratio=1.0, disrupted_modality=modality, seed=42,
            )
            acc, w = evaluate(model, DataLoader(ds, batch_size=16, shuffle=False), device)
            print(f"{modality:<8} {dtype.value:<4} {acc:8.4f} {w:8.4f}")

    print("参考 no_recovery: clean=0.7301 Image-D=0.7146 Text-D=0.5951")
