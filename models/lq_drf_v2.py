...
class LQDRFv2(nn.Module):
    """
    LQ-DRF-v2: 轻量质量感知 DRF 改进版
    - 保留特征队列 + 质量加权
    - 恢复：仅低质量时触发；图像恢复强、文本恢复极弱
    """
    def __init__(
        self,
        num_classes=3,
        ...
        recovery_threshold=0.35,  # 低于此质量才恢复
        img_recovery_alpha=0.4,   # 图像恢复强度
        txt_recovery_alpha=0.1,   # 文本恢复强度（故意很弱）
    ):
        super().__init__()
        self.recovery_threshold = recovery_threshold
        self.img_recovery_alpha = img_recovery_alpha
        self.txt_recovery_alpha = txt_recovery_alpha
    ...
    def forward(self, image, input_ids, attention_mask, update_queue=True):
        ...
        # 轻度平滑
        img_quality = 0.6 * img_quality + 0.4
        txt_quality = 0.6 * txt_quality + 0.4

        # ===== 条件恢复：仅低质量时启用 =====
        recovered_img = self.txt_to_img(txt_feat)
        recovered_txt = self.img_to_txt(img_feat)

        img_low = (img_quality < self.recovery_threshold).float().unsqueeze(1)
        txt_low = (txt_quality < self.recovery_threshold).float().unsqueeze(1)

        # 图像侧：质量低时用文本帮助恢复（较强）
        img_feat = img_feat + img_low * self.img_recovery_alpha * recovered_img * (
            1.0 - img_quality
        ).unsqueeze(1)

        # 文本侧：质量低时仅轻微恢复（很弱，避免污染）
        txt_feat = txt_feat + txt_low * self.txt_recovery_alpha * recovered_txt * (
            1.0 - txt_quality
        ).unsqueeze(1)
